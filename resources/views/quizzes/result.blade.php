@extends('layouts.app')
@section('title', 'Sınav Sonucu – LiftAcademy')
@section('content')
...
<div class="min-h-screen bg-[#F5F0E8]">
  <div class="max-w-[800px] mx-auto px-6 py-12">
    ...
    <div class="border-[3px] border-[#0A0A0A] mb-8 overflow-hidden" style="box-shadow:8px 8px 0 #0A0A0A">
      <div class="{{ $attempt->passed ? 'bg-[#CCFF00]' : 'bg-[#FF2D2D]' }} px-8 py-10 text-center">
        ...
        <p class="font-black text-xl uppercase tracking-tight {{ $attempt->passed ? 'text-[#0A0A0A]' : 'text-white' }}">
          {{ $attempt->passed ? '✓ BAŞARILI' : '✕ BAŞARISIZ' }}
        </p>
      </div>
      <div class="bg-[#0A0A0A] px-8 py-5 grid grid-cols-3 gap-4 text-center">
        <div>
          ...
          <p class="font-mono text-[9px] text-[#555] uppercase tracking-widest mt-1">DOĞRU</p>
        </div>
        <div>
          <p class="font-black text-2xl text-[#FF2D2D]">{{ $attempt->total_questions - $attempt->correct_count }}</p>
          <p class="font-mono text-[9px] text-[#555] uppercase tracking-widest mt-1">YANLIŞ</p>
        </div>
        <div>
          <p class="font-black text-2xl text-[#FFE000]">{{ $attempt->total_questions }}</p>
          <p class="font-mono text-[9px] text-[#555] uppercase tracking-widest mt-1">TOPLAM SORU</p>
        </div>
      </div>
    </div>
    ...
    <div class="border-[3px] border-[#FFE000] bg-[#FFE000] p-6 mb-8" style="box-shadow:6px 6px 0 #0A0A0A">
      <p class="font-black text-lg uppercase tracking-tight text-[#0A0A0A] mb-1">🏆 Sertifikanız Oluşturuldu!</p>
      <p class="text-sm font-medium text-[#0A0A0A] opacity-70 mb-3">
        Sertifika No: <strong>{{ $certificate->cert_number }}</strong>
      </p>
    ...
    <div class="border-[3px] border-[#0A0A0A] mb-8" style="box-shadow:4px 4px 0 #0A0A0A">
      <div class="bg-[#0A0A0A] px-5 py-3">
        <p class="font-black text-[#F5F0E8] text-xs uppercase tracking-widest">Soru Analizi</p>
      </div>
      ...
      <div class="px-5 py-4 border-b border-[#eee] last:border-b-0">
        ...
        <div class="ml-9 space-y-1.5">
          ...
          <div class="flex items-center gap-2 px-3 py-1.5 text-xs font-medium
            ...
            <span>{{ $opt }}</span>
            @if($isCorrectOpt)<span class="ml-auto font-black text-[10px]">DOĞRU</span>@endif
            @if($isGiven && !$isCorrectOpt)<span class="ml-auto font-black text-[10px]">SEÇİLEN</span>@endif
          </div>
          @endforeach
          @if($q->explanation)
    ...
    <div class="flex flex-wrap gap-3">
      <a href="{{ route('quizzes.show', $attempt->quiz->id) }}" class="btn-brut-dark text-xs py-3 px-6">← Sınava Dön</a>
      <a href="{{ route('quizzes.index') }}" class="btn-brut text-xs py-3 px-6">Tüm Sınavlar</a>
