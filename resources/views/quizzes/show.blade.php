...
<div class="min-h-screen bg-[#F5F0E8]">
  <div class="max-w-[900px] mx-auto px-6 py-12">
    ...
    <div class="flex items-center gap-2 mb-8 text-mono-sm text-[#888]">
      <a href="{{ route('quizzes.index') }}" class="hover:text-[#0A0A0A] transition-colors">{{ __('ui.quiz_breadcrumb') }}</a>
      <span>→</span>
    ...
    <div class="border-[3px] border-[#0A0A0A] mb-6" style="box-shadow:6px 6px 0 #0A0A0A">
      <div class="bg-[#0A0A0A] px-6 py-5 flex items-start justify-between gap-4">
        ...
        @if($passed)
        <span class="shrink-0 bg-[#CCFF00] text-[#0A0A0A] font-black text-[10px] uppercase tracking-widest px-3 py-1.5 border-[2px] border-[#CCFF00]">✓ {{ __('ui.passed') }}</span>
        @endif
      </div>
      <div class="bg-[#F5F0E8] px-6 py-5">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
          @foreach([
            [$quiz->questions->count(), __('ui.question_count_label')],
            [$quiz->passing_score.'%',  __('ui.passing_score_label')],
            [$quiz->attempts,           __('ui.attempt_rights')],
            [$quiz->time_limit ? $quiz->time_limit.' '.__('ui.minutes') : __('ui.unlimited'), __('ui.time_label')],
          ] as $stat)
    ...
    <div class="border-[3px] border-[#FF2D2D] bg-[#FF2D2D] text-white p-5 mb-6">
      <p class="font-black text-sm uppercase tracking-wide mb-1">🔒 {{ __('ui.quiz_locked') }}</p>
      <p class="text-sm font-medium opacity-90">{{ __('ui.quiz_locked_msg') }}</p>
      @if($quiz->course)
    ...
    @auth
      ...
      <div class="border-[3px] border-[#FFE000] bg-[#FFE000] p-4 mb-8 font-bold text-sm uppercase tracking-wide">
        ⚠ {{ __('ui.attempts_exhausted') }} %{{ $bestScore ?? 0 }}
      </div>
      @endif
    @else
    <div class="border-[3px] border-[#0A0A0A] p-5 mb-8 flex items-center justify-between gap-4">
      <p class="font-bold text-sm text-[#0A0A0A]">{{ __('ui.login_required') }}</p>
      <a href="{{ route('login') }}" class="btn-brut text-xs py-2.5 px-5">{{ __('ui.login_button') }} ↗</a>
    </div>
    @endauth
    ...
    <div class="border-[3px] border-[#0A0A0A]" style="box-shadow:4px 4px 0 #0A0A0A">
      <div class="bg-[#0A0A0A] px-5 py-3 flex items-center justify-between">
        <p class="font-black text-[#F5F0E8] text-xs uppercase tracking-widest">{{ __('ui.past_attempts') }}</p>
      ...
      <div class="flex items-center justify-between px-5 py-3 border-b border-[#eee] last:border-b-0">
        <div class="flex items-center gap-4">
          ...
          <span class="font-mono text-[10px] text-[#888]">{{ $att->correct_count }}/{{ $att->total_questions }} {{ __('ui.correct_of') }}</span>
        </div>
        <div class="flex items-center gap-3">
          ...
          <span class="font-black text-[10px] uppercase tracking-widest {{ $att->passed ? 'tag-lime' : 'tag-red' }}">
            {{ $att->passed ? __('ui.passed_label') : __('ui.failed_label') }}
          </span>
          <a href="{{ route('quizzes.result', $att->id) }}" class="font-mono text-[10px] text-[#888] hover:text-[#0A0A0A] border-b border-dashed border-[#ccc]">{{ __('ui.detail') }}</a>
        </div>
      </div>
      @endforeach
    </div>
