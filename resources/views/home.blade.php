@extends('layouts.app')
@section('title', 'LiftAcademy – Güvenli Operatör Yetiştir')
@section('content')
...
<div class="bg-[#F5F0E8] text-[#0A0A0A] min-h-screen">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-8 items-start mb-10">
      <div>
        <div class="flex flex-wrap items-center gap-3 mb-6">
          ...
          <span class="text-mono-sm text-[#888] mt-1">– Vinç LMS</span>
        </div>
        <h1 class="text-display text-[#0A0A0A] mb-6" style="font-family:Inter,sans-serif">
          GÜVENLİ<br>
          <span class="relative inline-block">
            <span class="relative z-10">OPERATÖR</span>
        ...
        <p class="text-lg text-[#555] max-w-xl font-medium leading-relaxed mb-8">
          Video eğitim · Quiz · Senaryo simülasyonu · Uluslararası sertifikasyon.<br>
          Vinç operatörlerini en yüksek güvenlik standartlarında yetiştiren kurumsal LMS.
        </p>
        <div class="flex flex-wrap gap-4">
          <a href="{{ route('register') }}" class="btn-brut text-base px-8 py-4">Hemen Başla ↗</a>
          <a href="{{ route('courses.index') }}" class="btn-brut-dark text-base px-8 py-4">Kurslara Bak →</a>
        </div>
      </div>
      <div class="hidden lg:flex flex-col gap-5 pt-4">
        <div class="card-brut-yellow w-52 animate-float">
          <p class="text-mono-sm mb-1">Platform İstatistiği</p>
          <p class="text-5xl font-black leading-none">97%</p>
          <p class="text-sm font-bold mt-1 uppercase tracking-wide">Geçme Oranı</p>
        </div>
      </div>
    </div>
    <div class="grid grid-cols-2 md:grid-cols-4 border-[3px] border-[#0A0A0A]" style="box-shadow:6px 6px 0 #0A0A0A">
      @foreach([['1.2K+','KAYITLI OPERATÖR',false],['135+','AKTİF KURS',true],['%97','GEÇME ORANI',false],['3.4K+','SERTİFİKA',false]] as $i => $s)
  ...
  <div class="mt-16">
    <div class="overflow-hidden border-y-[3px] border-[#0A0A0A] bg-[#FFE000] py-3 select-none">
      <div class="marquee-track">
        @foreach(array_fill(0, 4, ['VİNÇ EĞİTİMİ','★','SERTİFİKASYON','★','OPERATÖR YETİŞTİR','★','GÜVENLİ SAHA','★','LMS PLATFORM','★']) as $row)
  ...
  <section class="max-w-[1400px] mx-auto px-6 py-20">
    <div class="flex items-end justify-between mb-10">
      <h2 class="text-section">EĞİTİM<br>KATEGORİLERİ</h2>
      <a href="{{ route('courses.index') }}" class="btn-brut-dark text-xs py-2.5 hidden md:inline-flex">Tüm Kurslar ↗</a>
    </div>
    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
      @foreach([
        ['🦺','İSG & Güvenlik','SAFETY','bg-[#FF2D2D]','text-white'],
        ['🏗️','Vinç Türleri','CRANE_TYPE','bg-[#0047FF]','text-white'],
        ['⚙️','Operasyon','OPERATION','bg-[#FFE000]','text-[#0A0A0A]'],
        ['🔧','Teknik','TECHNICAL','bg-[#CCFF00]','text-[#0A0A0A]'],
        ['⚠️','Risk Yönetimi','RISK','bg-[#FF3CAC]','text-white'],
        ['🪪','Sertifikasyon','CERTIFICATION','bg-[#0A0A0A]','text-[#FFE000]'],
      ] as $cat)
      <a href="{{ route('courses.index', ['category' => $cat[2]]) }}" class="group flex flex-col justify-between p-6 {{ $cat[3] }} {{ $cat[4] }} border-[3px] border-[#0A0A0A] min-h-[140px] hover-lift">
        ...
        <div>
          ...
          <p class="text-xs font-bold uppercase tracking-wider opacity-60 mt-1">Keşfet ↗</p>
        </div>
      </a>
      @endforeach
    </div>
  ...
  <section class="border-t-[3px] border-[#0A0A0A]">
    <div class="max-w-[1400px] mx-auto px-6 py-20">
      <div class="flex items-center gap-6 mb-12">
        <h2 class="text-section">ÇALIŞMA<br>ÖRNEKLERİ</h2>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        @foreach([
          ['01','İSG Zorunlu Eğitim Sistemi','GÜVENLİK','1.200+ operatörün İSG sertifikasını 3 ayda tamamladığı otomatik müfredat sistemi.','%97 tamamlanma oranı','bg-[#FFE000]','text-[#0A0A0A]','tag-black','🦺'],
          ['02','Mobil Vinç Operatör Okulu','VİNÇ','86 dersten oluşan modüler eğitim. Simülasyon + quiz + sertifika entegrasyonu.','920 aktif öğrenci','bg-[#0047FF]','text-white','tag-yellow','🏗️'],
          ['03','Sertifikasyon Yol Haritası','SERTİFİKA','Junior\'dan Trainer\'a 5 kademeli uluslararası sertifikasyon programı.','3.400+ sertifika verildi','bg-[#CCFF00]','text-[#0A0A0A]','tag-black','🪪'],
          ['04','Risk Simülasyon Motoru','SİMÜLASYON','Devrilme, rüzgar ve elektrik hattı senaryolarında gerçek karar simülasyonu.','10 kritik senaryo','bg-[#FF2D2D]','text-white','tag-yellow','⚠️'],
        ] as $cs)
      ...
      <div class="flex flex-col md:flex-row items-start md:items-end justify-between gap-4 mb-8">
        <div>
          <div class="flex items-center gap-2 mb-3">
            <span class="tag-yellow text-[10px]">ENDÜSTRİYEL SİMÜLATÖR</span>
            <span class="tag-pink text-[10px]">● CANLI</span>
          </div>
          <h2 class="text-section text-[#F5F0E8]">VİNÇ KAPASİTE<br><span class="text-[#FFE000]">VİZÜALİZASYONU</span></h2>
          <p class="text-[#444] font-medium mt-3 max-w-xl leading-relaxed text-sm">Gerçek yük tablolarına dayalı interaktif kapasite simülatörü. Grafiğe tıklayarak yarıçap & yükseklik noktasını seçin, anlık kapasite hesabı sağda görünür.</p>
        </div>
        <div class="flex items-center gap-3">
          <span class="text-mono-sm text-[#333]">GÜVENLİK EŞİĞİ</span>
          ...
          <button id="cv-reset" class="border-[2px] border-[#FF2D2D] text-[#FF2D2D] font-black text-[10px] uppercase tracking-wider px-3 py-1.5 hover:bg-[#FF2D2D] hover:text-white transition-colors cursor-pointer">TEMİZLE</button>
        </div>
      </div>
      ...
      <div class="flex flex-col lg:flex-row border-[3px] border-[#FFE000]" style="box-shadow:6px 6px 0 #FFE000">
        ...
        <div class="flex-1 bg-[#0a0a0a] relative" style="min-height:460px">
          ...
          <div class="flex flex-wrap items-center gap-2 px-4 py-2 border-b border-[#222]">
            ...
            <div class="flex items-center gap-1.5">
              <span class="text-[#666] font-mono text-[9px] uppercase tracking-widest">MODEL</span>
            ...
            {{-- Bum --}}
            <div class="flex items-center gap-1.5">
              <span class="text-[#666] font-mono text-[9px] uppercase tracking-widest">BUM</span>
            ...
            {{-- Karşı ağırlık --}}
            <div class="flex items-center gap-1.5">
              <span class="text-[#666] font-mono text-[9px] uppercase tracking-widest">KARŞI AĞ.</span>
            ...
            {{-- Taşınacak yük --}}
            <div class="flex items-center gap-1.5">
              <span class="text-[#666] font-mono text-[9px] uppercase tracking-widest">YÜK</span>
            ...
            <div class="ml-auto">
              <button id="cv-multi" class="text-[#CCFF00] border border-[#CCFF00] font-bold text-[9px] uppercase tracking-wider px-2.5 py-1 transition-colors cursor-pointer" style="opacity:0.9">
                ✓ ÇOKLU MOD
              </button>
            </div>
          </div>
          ...
          <div class="flex flex-wrap items-center gap-x-5 gap-y-1.5 px-4 py-3 border-t border-[#1e1e1e]">
            ...
            <div class="flex items-center gap-2">
              ...
              <div>
                <p class="font-mono text-[10px] text-[#ddd] font-bold uppercase tracking-wide leading-none">KAPASİTE EĞRİSİ</p>
                <p class="font-mono text-[9px] text-[#555] leading-none mt-0.5">Her yarıçapta max kaldırılabilir yük</p>
              </div>
            </div>
            {{-- Güvenlik eşiği --}}
            <div class="flex items-center gap-2">
              ...
              <div>
                <p class="font-mono text-[10px] text-[#ddd] font-bold uppercase tracking-wide leading-none">GÜVENLİK EŞİĞİ</p>
                <p class="font-mono text-[9px] text-[#555] leading-none mt-0.5">Eğrinin %<span id="leg-thresh">80</span>'i — bu altı güvenli</p>
              </div>
            </div>
            {{-- Marker durumları --}}
            <div class="flex items-center gap-3">
              <div class="flex items-center gap-1.5"><div class="w-3 h-3 rounded-full bg-[#CCFF00]"></div><span class="font-mono text-[9px] text-[#bbb] uppercase">Güvenli</span></div>
              <div class="flex items-center gap-1.5"><div class="w-3 h-3 rounded-full bg-[#FFE000]"></div><span class="font-mono text-[9px] text-[#bbb] uppercase">Sınır</span></div>
              <div class="flex items-center gap-1.5"><div class="w-3 h-3 rounded-full bg-[#FF2D2D]"></div><span class="font-mono text-[9px] text-[#bbb] uppercase">Aşım</span></div>
            </div>
            <div class="ml-auto font-mono text-[9px] text-[#444] uppercase">Grafiğe tıkla → nokta ekle · Tekrar tıkla → sil</div>
          </div>
        </div>
        ...
        <div class="w-full lg:w-72 bg-[#0d0d0d] border-t-[3px] lg:border-t-0 lg:border-l-[3px] border-[#FFE000] flex flex-col">
          ...
          <div class="px-4 py-3 border-b border-[#1e1e1e] bg-[#111]">
            <p class="font-mono text-[9px] text-[#555] uppercase tracking-widest">CANLI HESAPLAMA</p>
          ...
          <div class="grid grid-cols-2 border-b border-[#1e1e1e]">
            <div class="px-3 py-2.5 border-r border-[#1e1e1e]">
              <p class="font-mono text-[9px] text-[#666] uppercase tracking-widest mb-1">KANCA YÜK.</p>
              <p id="cv-ph" class="font-black text-xl text-white leading-none">—</p>
              <p class="font-mono text-[9px] text-[#444] mt-1">METRE</p>
            </div>
            <div class="px-3 py-2.5">
              <p class="font-mono text-[9px] text-[#666] uppercase tracking-widest mb-1">ÇALIŞMA YAR.</p>
          ...
          <div class="grid grid-cols-2 border-b border-[#1e1e1e]">
            <div class="px-3 py-2.5 border-r border-[#1e1e1e]">
              <p class="font-mono text-[9px] text-[#666] uppercase tracking-widest mb-1">MAX KAPASİTE</p>
              <p id="cv-pcap" class="font-black text-xl text-[#FFE000] leading-none">—</p>
              <p class="font-mono text-[9px] text-[#444] mt-1">TON (O YARÇAPTA)</p>
            </div>
            <div class="px-3 py-2.5">
              <p class="font-mono text-[9px] text-[#666] uppercase tracking-widest mb-1">KALAN MARJ</p>
              <p id="cv-pmg" class="font-black text-xl text-white leading-none">—</p>
              <p class="font-mono text-[9px] text-[#444] mt-1">YÜK GİRİNCE DOLAR</p>
            </div>
          </div>
          ...
          <div class="px-3 py-2.5 border-b border-[#1e1e1e]">
            <div class="flex justify-between items-center mb-1.5">
              <span class="font-mono text-[9px] text-[#666] uppercase tracking-widest">KAP. KULLANIMI</span>
          ...
          {{-- Vinç özellikleri --}}
          <div class="px-3 py-2.5 border-b border-[#1e1e1e]">
            <p class="font-mono text-[9px] text-[#555] uppercase tracking-widest mb-2">VİNÇ ÖZELLİKLERİ</p>
            <div class="space-y-1.5">
              <div class="flex justify-between">
                <span class="font-mono text-[10px] text-[#666]">Bum Uzunluğu</span>
                <span id="cv-spec-boom" class="font-mono text-[10px] text-[#ccc] font-bold">42 m</span>
              </div>
              <div class="flex justify-between">
                <span class="font-mono text-[10px] text-[#666]">Karşı Ağırlık</span>
                <span id="cv-spec-cw" class="font-mono text-[10px] text-[#ccc] font-bold">14 ton</span>
              </div>
              <div class="flex justify-between">
                <span class="font-mono text-[10px] text-[#666]">Max. Yükseklik</span>
                <span id="cv-spec-mh" class="font-mono text-[10px] text-[#ccc] font-bold">60 m</span>
              </div>
              <div class="flex justify-between">
                <span class="font-mono text-[10px] text-[#666]">Max. Yarıçap</span>
                <span id="cv-spec-mr" class="font-mono text-[10px] text-[#ccc] font-bold">52 m</span>
              </div>
              <div class="flex justify-between">
                <span class="font-mono text-[10px] text-[#666]">Güvenlik Eşiği</span>
          ...
          <div class="flex-1 flex flex-col overflow-hidden" style="min-height:0">
            <div class="flex items-center justify-between px-3 py-2 border-b border-[#1e1e1e]">
              <span class="font-mono text-[9px] text-[#555] uppercase tracking-widest">SEÇİLİ NOKTALAR</span>
            ...
            <div id="cv-list" class="flex-1 overflow-y-auto divide-y divide-[#1a1a1a]">
              <p class="font-mono text-[10px] text-[#333] text-center py-5">Grafiğe tıklayarak<br>nokta ekleyin</p>
            </div>
          </div>
  ...
  <section class="border-t-[3px] border-[#0A0A0A] bg-[#0A0A0A]">
    <div class="max-w-[1400px] mx-auto px-6 py-20">
      <div class="flex items-end justify-between mb-12">
        <h2 class="text-section text-[#F5F0E8]">ÖNE ÇIKAN<br><span class="text-[#FFE000]">KURSLAR</span></h2>
        <a href="{{ route('courses.index') }}" class="btn-brut text-xs py-2.5 hidden md:inline-flex">Tümünü Gör ↗</a>
      </div>
      @if($featuredCourses->isEmpty())
      <div class="text-center py-16 border-[3px] border-[#F5F0E8]/20">
        <p class="font-black text-[#F5F0E8] uppercase tracking-tight mb-2">Henüz yayında kurs yok</p>
      </div>
      @else
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <a href="{{ route('courses.show', $c->slug) }}" class="group border-[3px] border-[#F5F0E8]/20 hover:border-[#FFE000] bg-[#111] p-6 flex flex-col gap-4 hover-lift-yellow">
          ...
          <div class="flex flex-wrap items-center gap-2 mt-auto">
            ...
            <span class="ml-auto text-[10px] font-bold text-[#555]">{{ $c->enrollments_count }} öğrenci</span>
          </div>
        </a>
        @endforeach
      </div>
  ...
  <section class="border-t-[3px] border-[#0A0A0A]">
    <div class="max-w-[1400px] mx-auto px-6 py-20">
      <h2 class="text-section mb-12 text-center">NASIL<br><span class="relative inline-block"><span class="relative z-10">ÇALIŞIR?</span><span class="absolute bottom-1 left-0 w-full h-4 bg-[#FFE000] -z-0"></span></span></h2>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-0 border-[3px] border-[#0A0A0A]" style="box-shadow:8px 8px 0 #0A0A0A">
        @foreach([
          ['01','▶','Video İzle','HD video dersler. Kendi hızında, mobilden veya masaüstünden.','bg-[#F5F0E8]','text-[#0A0A0A]',true],
          ['02','⚡','Quiz Geç','Çoktan seçmeli sorular, süre limiti, min. %70–90 puan.','bg-[#FFE000]','text-[#0A0A0A]',true],
          ['03','🏆','Sertifika Al','Otomatik sertifika. Benzersiz numara ile doğrulanabilir.','bg-[#0A0A0A]','text-[#F5F0E8]',false],
        ] as $i => $step)
  ...
  <section class="border-t-[3px] border-[#0A0A0A]">
    <div class="max-w-[1400px] mx-auto px-6 py-20">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        <div>
          <span class="tag-black mb-6 inline-block">5 KADEME</span>
          <h2 class="text-section mb-6">SERTİFİKASYON PROGRAMI</h2>
          <p class="text-[#555] font-medium leading-relaxed mb-8">Uluslararası standartlara uygun, Junior Operatör'den Trainer kademesine uzanan 5 aşamalı yetkinlik sistemi.</p>
          <a href="{{ route('courses.index', ['category'=>'CERTIFICATION']) }}" class="btn-brut text-base px-8 py-4">Programa Başla ↗</a>
        </div>
        <div class="space-y-0 border-[3px] border-[#0A0A0A]" style="box-shadow:8px 8px 0 #0A0A0A">
          @foreach([
            ['Junior Operatör',4,'bg-[#CCFF00]','text-[#0A0A0A]'],
            ['Operatör',7,'bg-[#FFE000]','text-[#0A0A0A]'],
            ['Senior Operatör',10,'bg-[#FF3CAC]','text-white'],
            ['Supervisor',14,'bg-[#FF2D2D]','text-white'],
            ['Trainer',18,'bg-[#0A0A0A]','text-[#FFE000]'],
          ] as $i => $lvl)
          <div class="{{ $lvl[2] }} {{ $lvl[3] }} p-5 flex items-center justify-between {{ $i < 4 ? 'border-b-[3px] border-[#0A0A0A]' : '' }}">
            ...
            <span class="text-mono-sm opacity-60">{{ $lvl[1] }} KURS</span>
          </div>
          @endforeach
        </div>
      </div>
  ...
  <section class="border-t-[3px] border-[#0A0A0A] bg-[#FFE000]">
    <div class="max-w-[1400px] mx-auto px-6 py-20">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div>
          <span class="tag-black mb-6 inline-block">KURUMSAL DEMO</span>
          <h2 class="text-section text-[#0A0A0A] mb-6">FİRMANIZ<br>İÇİN DEMO</h2>
          <p class="text-[#0A0A0A] font-medium leading-relaxed mb-8 opacity-70">Toplu operatör kaydı, özel eğitim modülleri ve sertifikasyon yönetimi için kurumsal paketi deneyin.</p>
          <div class="space-y-4">
            @foreach(['İSG uyumlu zorunlu müfredat','Toplu kayıt ve grup yönetimi','Otomatik sertifika sistemi'] as $item)
            <div class="flex items-center gap-3">
        ...
        <form class="bg-[#F5F0E8] border-[3px] border-[#0A0A0A] p-8 space-y-4" style="box-shadow:8px 8px 0 #0A0A0A" method="POST" action="#">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div><label class="text-mono-sm mb-1.5 block">AD SOYAD</label><input type="text" placeholder="Adınız" class="input-brut"></div>
            <div><label class="text-mono-sm mb-1.5 block">FİRMA</label><input type="text" placeholder="Firma adı" class="input-brut"></div>
          </div>
          <div><label class="text-mono-sm mb-1.5 block">E-POSTA</label><input type="email" placeholder="iletisim@firma.com" class="input-brut"></div>
          <div>
            <label class="text-mono-sm mb-1.5 block">OPERATÖR SAYISI</label>
            <select class="input-brut"><option>1–10</option><option>10–50</option><option>50–200</option><option>200+</option></select>
          </div>
          <div><label class="text-mono-sm mb-1.5 block">MESAJ</label><textarea rows="4" placeholder="İhtiyaçlarınızı anlatın..." class="input-brut resize-none"></textarea></div>
          <button type="submit" class="btn-brut-dark w-full justify-center py-4 text-sm">Demo Talep Et ↗</button>
        </form>
      </div>
    </div>
  </section>
    ...
    <div class="max-w-[1400px] mx-auto px-6 py-12">
      <div class="flex flex-col md:flex-row items-start justify-between gap-8 mb-10">
        <div>
          ...
          <p class="text-sm text-[#444] max-w-xs leading-relaxed">Vinç operatörü eğitim ve sertifikasyon platformu.</p>
        </div>
        <div class="grid grid-cols-3 gap-8 text-sm">
          @foreach([['Platform',['Kurslar','Sertifikasyon','Simülasyonlar','Admin']],['Roller',['Operatör','Supervisor','Eğitmen','Yönetici']],['Destek',['SSS','İletişim','Gizlilik','Koşullar']]] as $col)
          <div>
      ...
      <div class="border-t-[2px] border-[#1a1a1a] pt-6 flex flex-col md:flex-row items-center justify-between gap-3">
        <span class="text-mono-sm text-[#444]">© 2024 LiftAcademy – TÜM HAKLARI SAKLIDIR</span>
...
function statusOf(r, load) {
  ...
  if (maxC<=0) return { st:'ERİŞİM DIŞI', col:'#555' };
  const ratio = load/maxC*100;
  if (ratio > 100)       return { st:'AŞIM', col:'#FF2D2D' };
  if (ratio > safetyPct) return { st:'SINIRA YAKIN', col:'#FFE000' };
  return { st:'GÜVENLİ', col:'#CCFF00' };
}
...
function buildSVG() {
  ...
  if(vispts.length>1){
    ...
    s+=`<text x="${lx}" y="${ly}" fill="#FF6B00" font-size="8" font-family="monospace" opacity="0.7" font-weight="700">%${safetyPct} EŞİĞİ</text>`;
  }
  ...
  s+=`<text x="${PAD.l+w/2}" y="${H()-4}" fill="#777" font-size="10" font-family="monospace" text-anchor="middle" letter-spacing="3" font-weight="700">ÇALIŞMA YARÇAPI (m)</text>`;
  ...
  s+=`<text x="13" y="${PAD.t+h/2}" fill="#777" font-size="10" font-family="monospace" text-anchor="middle" font-weight="700" transform="rotate(-90 13 ${PAD.t+h/2})" letter-spacing="2">KAPASİTE (ton)</text>`;
...
function buildMarker(m) {
  ...
  if(isAct){
    ...
    g+=`<text x="${lx}" y="${ly+13}" fill="${col}" font-size="8" font-family="monospace" opacity="0.8">Maks: ${m.maxC.toFixed(1)}t — ${st}</text>`;
  } else {
...
function updatePanel() {
  ...
  if(!act){
    ...
    el('cv-pmg') && (el('cv-pmg').textContent=load<=maxC?margin:'AŞIM');
...
function setStatus(st,col){
  ...
  if(!st){ box.textContent='— NOKTAYA TIKLA —'; box.style.borderColor='#2a2a2a'; box.style.color='#3a3a3a'; return; }
...
function updateList(){
  ...
  if(!markers.length){
    el('cv-list').innerHTML='<p class="font-mono text-[10px] text-[#2a2a2a] text-center py-5">Grafiğe tıkla → nokta ekle</p>';
    return;
  }
  el('cv-list').innerHTML=markers.map(m=>{
    ...
    return `<div class="flex items-center justify-between px-3 py-2.5 cursor-pointer hover:bg-[#131313] border-l-2 transition-all ${isAct?'bg-[#141414]':''}" style="border-color:${isAct?col:'transparent'}" onclick="window.__cvSel(${m.id})">
      <div class="flex-1 min-w-0">
        ...
        <div class="flex items-center gap-2">
          <span class="font-mono text-[9px] text-[#555]">Maks: ${m.maxC.toFixed(1)}t</span>
...
function showTip(e, r, load, maxC){
  ...
  tipEl.querySelector('#cv-tip-r').textContent=`Yarıçap: ${r} m  |  Kanca: ${hookH} m`;
  tipEl.querySelector('#cv-tip-h').textContent=`Seçilen yük: ${load.toFixed(1)} ton`;
  tipEl.querySelector('#cv-tip-cap').textContent=`Maks. kapasite: ${maxC.toFixed(1)} ton`;
  tipEl.querySelector('#cv-tip-mg').textContent=`Kullanım: %${pct} ${load>maxC?'⚠ AŞIM':''}`;
...
function init(){
  ...
  document.getElementById('cv-multi')?.addEventListener('click',function(){
    multiMode=!multiMode;
    this.textContent=multiMode?'✓ ÇOKLU MOD':'◻ TEK NOKTA';
